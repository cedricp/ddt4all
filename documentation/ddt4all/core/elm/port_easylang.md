# Port, In Simple English

Source: `src/ddt4all/core/elm/port.py`

`Port` is one part of the core code. This version uses simple English. It keeps the same meaning as the normal document, but uses shorter sentences.
...
Creates a `Port` object and sets its starting state.
...
This is the short version of how `Port` is used.
