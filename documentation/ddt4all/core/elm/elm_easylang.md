# ELM, In Simple English

Source: `src/ddt4all/core/elm/elm.py`

`ELM` is one part of the core code. This version uses simple English. It keeps the same meaning as the normal document, but uses shorter sentences.
...
Creates a `ELM` object and sets its starting state.
...
Cleans up the `ELM` object when it is destroyed.
...
Sends data or a command through the current connection.
...
Closes the current connection or protocol.
...
This is the short version of how `ELM` is used.
